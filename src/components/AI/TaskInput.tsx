...
export function TaskInput() {
  ...
  return (
    <div className={`${state.theme === 'dark'
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="relative">
          <textarea
            ...
            className={`w-full p-4 pr-12 rounded-xl border-2 resize-none transition-all duration-200 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 ${state.theme === 'dark'
        ...
        {isListening && (
          <div className="flex items-center space-x-2 px-4 py-3 bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-200 dark:border-red-800 animate-pulse">
            ...
            <span className={`text-sm font-medium ${state.theme === 'dark' ? 'text-red-300' : 'text-red-700'
              }`}>
        ...
        {!displayText && (
          <div className="space-y-2">
            <p className={`text-xs font-medium ${state.theme === 'dark' ? 'text-gray-400' : 'text-gray-600'
              }`}>
              💡 Quick suggestions:
            </p>
            <div className="flex flex-wrap gap-2">
              {quickSuggestions.slice(0, 3).map((suggestion, index) => (
                <button
                  ...
                  className={`text-xs px-3 py-1 rounded-full border transition-all duration-200 hover:scale-105 ${state.theme === 'dark'
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center space-x-4 flex-wrap">
            <div className="flex items-center space-x-2 whitespace-nowrap relative group">
              <Flag className={`w-4 h-4 flex-shrink-0 ${state.theme === 'dark' ? 'text-purple-400' : 'text-purple-600'
                }`} />
              <select
                value={priority}
                onChange={(e) => setPriority(e.target.value as any)}
                className={`text-sm font-medium bg-transparent border-none focus:ring-0 cursor-pointer pr-2 py-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors ${state.theme === 'dark' ? 'text-gray-400' : 'text-gray-600'
                  }`}
            ...
            <div className="flex items-center space-x-2 whitespace-nowrap relative group">
              <Clock className={`w-4 h-4 flex-shrink-0 ${state.theme === 'dark' ? 'text-blue-400' : 'text-blue-600'
                }`} />
              <select
                value={estimatedTime}
              ...
              >
                ...
                <option value="15">15 mins (ASAP)</option>
            ...
            <div className="flex items-center space-x-2 whitespace-nowrap">
              <Tag className={`w-4 h-4 flex-shrink-0 ${state.theme === 'dark' ? 'text-green-400' : 'text-green-600'
                }`} />
              <span className={`text-sm font-medium ${state.theme === 'dark' ? 'text-gray-400' : 'text-gray-600'
                }`}>
                Auto-tagging
              </span>
            </div>
          ...
          <button
            ...
            className={`px-6 py-3 bg-gradient-to-r from-purple-500 via-indigo-500 to-blue-500 text-white rounded-xl font-semibold transition-all duration-200 flex items-center space-x-2 shadow-lg flex-shrink-0 ${!displayText.trim() || isProcessing
      ...
      {/* AI Status Indicator */}
      <div className={`mt-4 pt-4 border-t ${state.theme === 'dark' ? 'border-gray-700' : 'border-gray-200'
        }`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
            <span className={`text-xs ${state.theme === 'dark' ? 'text-gray-400' : 'text-gray-600'
              }`}>
              AI Assistant Active
            </span>
          </div>
          <span className={`text-xs ${state.theme === 'dark' ? 'text-gray-500' : 'text-gray-500'
            }`}>
